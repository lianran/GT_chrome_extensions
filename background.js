...
function updateTab(tab) {
  ...
  if (proto === 'https') {
    ...
    if (hostname in cachedValidatonData) {
      displayPageInfo(tabId, proto, false, cachedValidatonData[hostname], hostname)
      return;
    }

    // Fetch
    displayPageInfo(tabId, proto, true, null, hostname);
    fetchCertInfo(hostname, function(data) {
      // Store response
      if (data !== null) {
        cachedValidatonData[hostname] = data;
      }
      displayPageInfo(tabId, proto, false, data, hostname);
    })
  ...
  displayPageInfo(tabId, proto, false, nullm, hostname);
}

// Display page info
function displayPageInfo(tabId, pageProtocol, loading, validationData, hostname) {
  if (loading) {
    updateBadge(tabId, colors['gray'], '...');
    updatePopupData(tabId, null, colors['gray'], 'Loading...', 'Loading validation data, try opening this popup again.', hostname);
    return;
  }
  ...
  if (pageProtocol === 'http') {
    ...
    updatePopupData(tabId, null, colors['orange'], 'HTTP Page', 'Data sent to / received from this site is transmitted in plaintext.', hostname);
  } else if (pageProtocol === 'https') {
    // HTTPS
    // If failed to fetch data
    if (validationData === null) {
      updateBadge(tabId, colors['red'], '!');
      updatePopupData(tabId, null, colors['red'], 'Data fetch error', 'Try reloading the page. Note that this extension only works with publicly accessible sites.', hostname);
      return;
    }
    // Display data
    updateBadge(tabId, validationData['result_color_hex'], validationData['validation_result_short']);
    updatePopupData(tabId, validationData, null, null, null, hostname);
  } else {
...
// Update popup data
function updatePopupData(tabId, data, color, validationResult, message, hostname) {
  if (data !== null) {
    popupData[tabId] = data;
    popupData[tabId]['hostname'] = hostname;
  } else {
    popupData[tabId] = {};
